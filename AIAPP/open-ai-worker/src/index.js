...
document.getElementById('ticker-input-form').addEventListener('submit', (e) => {
    ...
    const inputValue = tickerInput.value.trim().toUpperCase()
    ...
    if (tickersArr.includes(inputValue)) {
        label.style.color = 'red'
        label.textContent = `${inputValue} is already added.`
        tickerInput.value = ''
        return
    }
    ...
    if (tickersArr.length >= 3) {
        label.style.color = 'red'
        label.textContent = 'You can only add up to 3 tickers.'
        tickerInput.value = ''
        return
    }
    
    // Add ticker
    tickersArr.push(inputValue)
...
async function fetchStockData() {
    // Validate that tickers are added
    if (tickersArr.length === 0) {
        alert('Please add at least one ticker before generating a report.')
        return
    }
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            // TODO: Replace YOUR_POLYGON_API_KEY with your actual Polygon API key
            // For now, this is a placeholder - you'll need to get your API key from polygon.io
            const polygonApiKey = 'c5qsPTbzzpdnj9rKhptSe7kuwGy_U23w'
        ...
        loadingArea.innerText = 'There was an error fetching stock data. Check console for details.'
...
async function fetchReport(data) {
    ...
    try {
        const url = 'https://open-ai-worker.bigfave.workers.dev'
        ...
        alert(`Unable to access AI: ${err.message}. Please check your worker URL and try again.`)
    }
}
